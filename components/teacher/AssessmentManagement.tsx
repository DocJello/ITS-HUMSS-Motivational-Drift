...
const AssessmentManagement: React.FC<AssessmentManagementProps> = ({ assessments, onUpdateAssessments, topics }) => {
    ...
    const availableQuestions = useMemo(() => {
        ...
        return QUESTION_BANK.filter(q => q.topicId === formData.topicId && !selectedIds.has(q.id));
    }, [formData.topicId, formData.questionIds]);

    const currentQuestions = useMemo((): Question[] => {
        if (!formData.questionIds) return [];
        return formData.questionIds.map(id => QUESTION_BANK.find(q => q.id === id)).filter((q): q is Question => q !== undefined);
