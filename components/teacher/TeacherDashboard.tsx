...
import { User, AssessmentAttempt, Section, Assessment, Topic } from '../../types';
...
const TeacherDashboard: React.FC<TeacherDashboardProps> = ({ user, onLogout, allAttempts, assessments, onUpdateAssessments, topics, onUpdateTopics }) => {
    ...
    const renderContent = () => {
        switch (activePage) {
            case 'Sections':
                return <Sections allAttempts={allAttempts} teacherSections={teacherSections} assessments={assessments} topics={topics} />;
            case 'Lesson Management':
                return <LessonManagement topics={topics} onUpdateTopics={onUpdateTopics} assessments={assessments} />;
            case 'Question Bank':
                return <QuestionBank topics={topics} />;
            case 'Assessments':
                return <AssessmentManagement assessments={assessments} onUpdateAssessments={onUpdateAssessments} topics={topics} />;
            case 'Insights':
                return <SectionInsights allAttempts={allAttempts} teacherSections={teacherSections} assessments={assessments} />;
            default:
                return <Sections allAttempts={allAttempts} teacherSections={teacherSections} assessments={assessments} topics={topics} />;
        }
    };
