...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout, allAttempts, users, onUpdateUsers }) => {
    const [activePage, setActivePage] = useState('Model Performance');

    const renderContent = () => {
        switch (activePage) {
            case 'User Management':
                return <UserManagement users={users} onUpdateUsers={onUpdateUsers} />;
