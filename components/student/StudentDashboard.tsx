...
const StudentDashboard: React.FC<StudentDashboardProps> = ({ user, onLogout, allAttempts, onAddAttempt, assessments, topics }) => {
    ...
    const startAssessment = (assessmentId: string) => {
        ...
        if (assessment) {
            const questions = assessment.questionIds.map(id => QUESTION_BANK.find(q => q.id === id)).filter(Boolean) as Question[];
            setCurrentAssessment({ assessmentId, questions });
        }
    };
    ...
    const renderContent = () => {
        ...
        switch (activePage) {
            ...
            case 'My Results':
                return <ResultsDashboard studentAttempts={studentAttempts} assessments={assessments} topics={topics} />;
            default:
                return <Topics onStartAssessment={startAssessment} studentAttempts={studentAttempts} assessments={assessments} topics={topics} />;
        }
    };
